...
use \Trunghieuct\Console\Guardfile;
use \Mockery as m;
...
class GuardfileTest extends \PHPUnit_Framework_TestCase {
	...
	{
		$guardFile = m::mock('Trunghieuct\Console\Guardfile')->makePartial();
	...
	/**
	 * @expectedException Trunghieuct\Console\FileNotFoundException
	 */
	...
	public function testCompileStub()
	{
		...
		$guardFile = m::mock('Trunghieuct\Console\Guardfile', [$file, $config])->makePartial();
		$guardFile->shouldReceive('getConfigOption')
				  ->with('/_path$/i')
				  ->times(2)
